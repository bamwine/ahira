...
<?php
	//$query=mysqli_query($con,"select SUM(payment) as payment, SUM(interest) as interest from payment where date(payment_date)>='$start' and date(payment_date)<='$end' and branch_id='$branch'")or die(mysqli_error($con));
		$query=mysqli_query($con,"select * from payment  where date(payment_date)>='$start' and date(payment_date)<='$end' and branch_id='$branch'")or die(mysqli_error($con));
  $counter=1;
...
?>
      <tr>
        ...
        <td><?php 
		 ...
		 echo $bad=($count==1)? $rowd['cust_last'].", ".$rowd['cust_first']: "Normal Pay";
