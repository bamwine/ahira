...
include('../dist/includes/dbcon.php');
	...
	$cid=$_REQUEST['cid'];
		...
		mysqli_query($con,"INSERT INTO sales(cust_id,user_id,discount,amount_due,total,date_added,modeofpayment,cash_tendered,cash_change,branch_id,saletype) 
	VALUES('$cid','$id','$discount','$amount_due','$total','$date','cash','$tendered','$change','$branch','$saletype')")or die(mysqli_error($con));
		...
		while ($row=mysqli_fetch_array($query))
		{
			...
			mysqli_query($con,"INSERT INTO sales_details(prod_id,qty,price,sales_id) VALUES('$pid','$qty','$price','$sales_id')")or die(mysqli_error($con));
			 //if (($saletype!='Invoice')&&($saletype!='Quatation')){
				  if (($saletype=='Receipt')|| $saletype=='Credit'){				 
